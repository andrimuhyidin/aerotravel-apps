...
export function HorizontalDatePicker({
  ...
  useEffect(() => {
    if (selectedRef.current && scrollContainerRef.current) {
      ...
      const scrollLeft = selected.offsetLeft - container.offsetLeft - (container.clientWidth / 2) + (selected.clientWidth / 2);
    ...
    if (scrollContainerRef.current && value === 'all') {
      ...
      if (todayIndex > -1 && scrollContainerRef.current) {
        setTimeout(() => {
          const container = scrollContainerRef.current;
          if (container) {
            const todayButton = container.querySelector(`[data-date="${today}"]`) as HTMLElement;
            if (todayButton) {
              const scrollLeft = todayButton.offsetLeft - container.offsetLeft - (container.clientWidth / 2) + (todayButton.clientWidth / 2);
              container.scrollTo({
                left: scrollLeft,
                behavior: 'smooth',
              });
    ...
    <div className={cn('relative', className)}>
      ...
      >
        {dates.map((dateOption) => {
          ...
          const isTodayDate = dateOption.date ? isToday(dateOption.date) : false;
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isTodayDate && !isSelected && 'ring-2 ring-emerald-300 bg-emerald-50',
              )}
            >
              {dateOption.value === 'all' ? (
                <div className="flex flex-col items-center gap-0">
                  <CalendarIcon className="h-3.5 w-3.5" />
                  <span className="text-[9px] font-semibold leading-tight">{dateOption.label}</span>
                </div>
              ) : (
                <div className="flex flex-col items-center gap-0">
                  <span className="text-[9px] font-medium uppercase text-slate-500 leading-tight">
                    {dateOption.date?.toLocaleDateString('id-ID', { weekday: 'short' })}
                  </span>
                  <span className="text-sm font-bold leading-none">{dateOption.date?.getDate()}</span>
                  <span className="text-[9px] font-medium text-slate-500 leading-tight">
                    {dateOption.date?.toLocaleDateString('id-ID', { month: 'short' })}
                  </span>
                  {isTodayDate && (
                    <span className="mt-0.5 text-[8px] font-semibold text-emerald-600 leading-tight">Hari Ini</span>
                  )}
                </div>
              )}
            </button>
        ...
        >
          <div className="flex flex-col items-center gap-0">
            ...
            <span className="text-[9px] font-semibold leading-tight">Lainnya</span>
          </div>
        </button>
      </div>
          ...
          <div className="py-4">
            ...
            <div className="mb-4 flex gap-2">
              <Button
                ...
                className={cn(
                  'flex-1',
                  pickerMode === 'single' && 'bg-emerald-600 hover:bg-emerald-700',
                )}
              >
                Tanggal Tunggal
              </Button>
              <Button
                ...
                className={cn(
                  'flex-1',
                  pickerMode === 'range' && 'bg-emerald-600 hover:bg-emerald-700',
                )}
              >
                Rentang Tanggal
              </Button>
              ...
              {pickerMode === 'single' && selectedDate && (
                <div className="rounded-lg bg-emerald-50 p-3 text-sm">
                  <span className="font-medium text-emerald-900">
                    Tanggal dipilih: {format(selectedDate, 'EEEE, d MMMM yyyy', { locale: id })}
                  </span>
                </div>
              )}
                ...
                <div className="rounded-lg bg-emerald-50 p-3 text-sm">
                  {dateRange.from && (
                    <div className="font-medium text-emerald-900">
                      Dari: {format(dateRange.from, 'EEEE, d MMMM yyyy', { locale: id })}
                    </div>
                  )}
                  {dateRange.to && (
                    <div className="mt-1 font-medium text-emerald-900">
                      Sampai: {format(dateRange.to, 'EEEE, d MMMM yyyy', { locale: id })}
                    </div>
                  )}
            ...
            <Button
              onClick={() => {
                if (pickerMode === 'single' && selectedDate) {
                  ...
                  setCustomDateDialogOpen(false);
                  setSelectedDate(undefined);
                } else if (pickerMode === 'range' && dateRange?.from && dateRange?.to) {
