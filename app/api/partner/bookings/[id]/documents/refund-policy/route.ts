/**
 * API: Generate Refund Policy PDF
 * GET /api/partner/bookings/[id]/documents/refund-policy
 */

import { withErrorHandler } from '@/lib/api/error-handler';
import { createClient } from '@/lib/supabase/server';
import { generateRefundPolicyPDF } from '@/lib/pdf/refund-policy';
...
export const GET = withErrorHandler(async (
  ...
  try {
    ...
    const refundPolicyData = {
      ...
      language,
      customPolicy: whitelabel?.refund_policy || undefined,
    };

    const pdfBuffer = await generateRefundPolicyPDF(refundPolicyData);

    return new NextResponse(pdfBuffer as unknown as BodyInit, {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': `attachment; filename="refund-policy-${booking.booking_code}.pdf"`,
      },
    });
  } catch (error) {
    logger.error('Failed to generate refund policy PDF', error, {
      bookingId,
      userId: user.id,
    });
    throw error;
