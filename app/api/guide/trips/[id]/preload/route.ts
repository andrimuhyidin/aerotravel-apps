...
export const GET = withErrorHandler(
  async (_request: NextRequest, context: RouteContext) => {
    ...
    if (!branchContext.isSuperAdmin && branchContext.branchId) {
      legacyQuery = legacyQuery.eq('branch_id', branchContext.branchId);
    }

    const { data: legacyAssignment } = await legacyQuery.maybeSingle();
    ...
    try {
      // Get trip bookings (no branch_id filtering - trip already filtered)
      ...
      if (bookingIds.length > 0) {
        // Get passengers (no branch_id - filtered via booking_id)
        const { data: passengers } = await client
          .from('booking_passengers')
          .select('id, booking_id, full_name, phone, passenger_type')
          .in('booking_id', bookingIds);

        // Get manifest checks status (no branch_id - filtered via trip_id)
    ...
    try {
      let attendanceQuery = client
        .from('trip_guides')
        .select(
          'check_in_at, check_in_lat, check_in_lng, check_out_at, check_out_lat, check_out_lng, is_late'
        )
        .eq('trip_id', tripId)
        .eq('guide_id', user.id);
      ...
      const { data: attendanceData } = await attendanceQuery.maybeSingle();

      if (attendanceData) {
        attendance = {
          checkInAt: attendanceData.check_in_at,
          checkInLocation:
            attendanceData.check_in_lat && attendanceData.check_in_lng
              ? {
                  lat: attendanceData.check_in_lat,
                  lng: attendanceData.check_in_lng,
                }
              : null,
          checkOutAt: attendanceData.check_out_at,
          checkOutLocation:
            attendanceData.check_out_lat && attendanceData.check_out_lng
              ? {
                  lat: attendanceData.check_out_lat,
                  lng: attendanceData.check_out_lng,
                }
              : null,
    ...
    try {
      let expensesQuery = client
        .from('trip_expenses')
        .select(
          'id, category, description, total_amount, receipt_url, created_at'
        )
        .eq('trip_id', tripId)
        .order('created_at', { ascending: false });

      if (!branchContext.isSuperAdmin && branchContext.branchId) {
        expensesQuery = expensesQuery.eq('branch_id', branchContext.branchId);
      }
    ...
    logger.info('Trip preload successful', {
      tripId,
