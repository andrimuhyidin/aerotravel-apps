...
export const GET = withErrorHandler(async (
  ...
  if (error) {
    logger.error('Failed to fetch consents', error, { tripId, guideId: user.id });
    return NextResponse.json({ error: 'Failed to fetch consents' }, { status: 500 });
  }

  // Check if all passengers consented
  const { data: allConsented } = await client.rpc('all_passengers_consented', {
    trip_uuid: tripId,
  });
...
export const POST = withErrorHandler(async (
  ...
  if (existing) {
    ...
    const { data, error } = await withBranchFilter(
      client.from('passenger_consents'),
      branchContext,
    )
      .update(consentData)
      .eq('id', existing.id)
    ...
    if (error) {
      logger.error('Failed to update consent', error, { tripId, passengerId: payload.passenger_id });
    ...
    if (error) {
      logger.error('Failed to create consent', error, { tripId, passengerId: payload.passenger_id });
