    ...
    try {
      ...
      if (checklistData && checklistData.length > 0) {
        ...
        checklistData.forEach(
          (item: { facility_code: string; checked: boolean }) => {
      ...
      logger.info(
        'trip_facility_checklist table not available, trying JSONB field',
        {
          tripId,
          guideId: user.id,
        }
    ...
    try {
      ...
      if (trip?.facility_checklist) {
        return NextResponse.json({
          checklist: trip.facility_checklist as Record<string, boolean>,
        });
      }
    } catch (error) {
...
export const PUT = withErrorHandler(
  async (
    ...
    if (!crewAssignment && !legacyAssignment) {
      return NextResponse.json(
        { error: 'Forbidden - Not assigned to this trip' },
        { status: 403 }
      );
    }
    ...
    try {
      ...
      if (existing) {
        ...
        // Insert new
        const { error } = await client.from('trip_facility_checklist').insert({
          trip_id: tripId,
          guide_id: user.id,
      ...
      logger.info(
        'trip_facility_checklist table not available, using JSONB fallback',
        {
          tripId,
          guideId: user.id,
        }
      ...
      try {
        ...
        const currentChecklist =
          (trip?.facility_checklist as Record<string, boolean>) || {};
