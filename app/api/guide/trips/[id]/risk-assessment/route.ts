...
export const POST = withErrorHandler(
  async (
    ...
    if (payload.use_weather_data && payload.latitude && payload.longitude) {
      try {
        ...
        if (weatherResponse.ok) {
          ...
          if (
            !finalPayload.weather_condition &&
          ...
          if (!finalPayload.wave_height && finalPayload.wind_speed) {
            ...
            finalPayload.wave_height = Math.min(
              2.5,
              finalPayload.wind_speed / 20
            );
          }
        }
      } catch (error) {
        logger.warn('Failed to fetch weather data, using manual input', {
          error,
        });
      }
    }
    ...
    const { data: riskScore, error: scoreError } = await client.rpc(
    ...
    if (insertError) {
      logger.error('Failed to create risk assessment', insertError, {
        tripId,
        guideId: user.id,
      });
      return NextResponse.json(
