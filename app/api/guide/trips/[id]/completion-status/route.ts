...
export const GET = withErrorHandler(
  async (
    ...
    const checklist: {
      ...
      logisticsHandoverCompleted: {
        done: boolean;
        inboundHandoverId: string | null;
        applicable: boolean;
      };
    ...
    try {
      ...
      if (totalPax === 0) {
        ...
        checklist.allPassengersReturned.done =
          checklist.allPassengersReturned.current >= totalPax;
    ...
    checklist.documentationUploaded.done = Boolean(
      trip.documentation_url && trip.documentation_url.trim() !== ''
    );
    ...
    try {
      ...
      if (totalHandovers === 0) {
        ...
        checklist.logisticsHandoverCompleted.inboundHandoverId =
          inboundHandover?.id ?? null;
        checklist.logisticsHandoverCompleted.done =
          inboundHandover?.status === 'completed' &&
          inboundHandover?.verified_by_both === true;
    ...
    try {
      ...
      const checkOutTime =
        tripGuide?.check_out_at || attendance?.check_out_at || null;
    ...
    try {
      ...
      const requiredTasks = (tasks ?? []).filter(
        (t: { required: boolean }) => t.required === true
      );
      ...
      if (requiredTasks.length === 0) {
        ...
        if (!checklist.requiredTasksCompleted.done) {
          missingItems.push(
            `Required tasks belum selesai (${pendingTasks.length} pending)`
          );
        }
      }
    } catch (error) {
    ...
    try {
      const { count: expensesCount } = await client
        .from('guide_expenses')
        .select('id', { count: 'exact', head: true })
        .eq('trip_id', tripId);
      ...
      if (checklist.expensesSubmitted.warning) {
        warnings.push(
          'Expenses belum di-submit. Pastikan submit untuk reimbursement.'
        );
      }
    } catch (error) {
    ...
    try {
      ...
      const { count: crewCount } = await client
        .from('trip_crews')
        .select('id', { count: 'exact', head: true })
        .eq('trip_id', tripId)
      ...
      if (!isMultiGuide) {
        ...
        const { count: splitCount } = await client
          .from('trip_payment_splits')
          .select('id', { count: 'exact', head: true })
          .eq('trip_id', tripId);

        checklist.paymentSplitCalculated.done = (splitCount ?? 0) > 0;
        checklist.paymentSplitCalculated.warning =
          !checklist.paymentSplitCalculated.done;
