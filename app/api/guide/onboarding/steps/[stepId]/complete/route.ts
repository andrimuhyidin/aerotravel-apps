...
export const POST = withErrorHandler(async (request: NextRequest, { params }: RouteParams) => {
  ...
  try {
    ...
    if (completionError) {
      logger.error('Failed to complete step', completionError, { guideId: user.id, stepId });
    ...
    logger.error('Failed to complete onboarding step', error, { guideId: user.id, stepId });
    return NextResponse.json({ error: 'Failed to complete step' }, { status: 500 });
  }
});
