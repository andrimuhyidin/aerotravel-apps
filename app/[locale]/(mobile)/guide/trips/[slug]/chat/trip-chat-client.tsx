...
const QUICK_TEMPLATES = [
  {
    ...
    message:
      'Halo Ops, ada tamu yang terlambat datang. Mohon konfirmasi apakah perlu menunggu atau langsung berangkat?',
  },
  {
    type: 'bad_weather' as const,
    label: 'Cuaca Buruk',
    message:
      'Halo Ops, kondisi cuaca saat ini kurang baik. Mohon instruksi apakah trip tetap berjalan atau ditunda?',
  },
  {
    type: 'boat_equipment_issue' as const,
    label: 'Masalah Kapal/Alat',
    message:
      'Halo Ops, ada masalah dengan kapal/peralatan. Mohon bantuan atau instruksi lebih lanjut.',
  },
];
  ...
  const {
    ...
    queryFn: async ({ pageParam }) => {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to load messages (${res.status})`
        );
      }
      ...
      try {
        ...
        responseData = text
          ? JSON.parse(text)
          : { messages: [], hasMore: false, nextCursor: null, totalCount: 0 };
      } catch (e) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload) => {
          logger.info('[Chat] New message received via Realtime', {
            tripId,
      ...
      .subscribe((status) => {
        if (status === 'SUBSCRIBED') {
          ...
          logger.error('[Chat] Realtime subscription error', {
            tripId,
            status,
          });
        }
  ...
  const sendMutation = useMutation({
    mutationFn: async ({
      ...
      if (!isOnline) {
        ...
        if (file) {
          throw new Error(
            'File uploads require internet connection. Please connect to internet and try again.'
          );
        }
        ...
        const userRole =
          (userProfile as { role: string } | null)?.role || 'guide';
    ...
    <div className="flex h-[calc(100vh-8rem)] flex-col overflow-hidden">
      ...
      <div className="sticky top-0 z-10 border-b border-slate-200 bg-white px-4 py-3">
        ...
        <p className="mt-1 text-sm text-slate-600">
          Komunikasi dengan Ops untuk trip ini
        </p>
      </div>
            ...
            <div className="space-y-2">
              {QUICK_TEMPLATES.map((template) => (
                <button
                  ...
                  className="block w-full rounded-lg border border-slate-200 bg-white p-2.5 text-left text-xs transition-colors hover:bg-slate-50"
                >
                ...
                >
                        ...
                        <img
                          ...
                          onError={(e) => {
                            ...
                            if (target.parentElement) {
                              target.parentElement.textContent = senderName
                                .charAt(0)
                                .toUpperCase();
                            }
                          }}
                  ...
                  >
                    ...
                    {msg.attachmentUrl && (
                      <div
                        className={cn(
                          'mt-2 max-w-full overflow-hidden rounded-xl shadow-sm',
                          isGuide
                      ...
                      >
                        {msg.attachmentType === 'image' ? (
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-2 p-3 transition-colors hover:bg-slate-50"
                          >
      ...
      <div className="sticky bottom-0 space-y-2 border-t border-slate-200 bg-white px-4 py-3">
        <Button
          type="button"
          variant="outline"
          size="sm"
        ...
        <div className="flex items-end gap-2">
          ...
          <label
            ...
            className="flex h-10 w-10 flex-shrink-0 cursor-pointer items-center justify-center rounded-full border border-slate-300 bg-white transition-colors hover:bg-slate-50"
          >
          ...
          <div className="min-w-0 flex-1">
            <Textarea
              ...
              className="max-h-32 min-h-[44px] resize-none rounded-2xl border-slate-300 text-sm focus:border-emerald-500 focus:ring-emerald-500"
              rows={1}
            />
          </div>
          <Button
            onClick={handleSend}
            disabled={
              (!messageText.trim() && !selectedFile) || sendMutation.isPending
            }
            className="h-10 w-10 flex-shrink-0 rounded-full bg-emerald-600 p-0 hover:bg-emerald-700 disabled:opacity-50"
            size="lg"
          >
